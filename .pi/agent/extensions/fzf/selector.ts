...
function getBindings(): {
  ...
  const tui = PiTui as unknown as {
    ...
    getEditorKeybindings?: () => { matches: (data: string, keybinding: string) => boolean };
  };
  ...
  if (typeof tui.getEditorKeybindings === "function") {
    return tui.getEditorKeybindings();
  }
...
export class FuzzySelector implements Focusable {
  private input: InstanceType<typeof Input>;
  ...
  handleInput(data: string): void {
    ...
    // Navigation: up/down (uses select keybindings)
    ...
    // Select (uses confirm keybinding)
  ...
  render(width: number): string[] {
    ...
    if (this.previewTemplate) {
      ...
      // Help line
      const upKey = prettyKey("up");
      const downKey = prettyKey("down");
      const confirmKey = prettyKey("enter");
      const cancelKey = prettyKey("escape/ctrl+c");
      const previewUpKey = prettyKey(this.settings.previewScrollUp);
      const previewDownKey = prettyKey(this.settings.previewScrollDown);
      const helpText = this.previewTemplate
        ? ` ${upKey} ${downKey} nav • ${confirmKey} select • ${cancelKey} cancel • ${previewUpKey}/${previewDownKey} scroll preview`
