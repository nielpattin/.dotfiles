...
function getSkillSummary(r: SingleResult): string | null {
	const skillLoad = r.skillLoad;
	if (!skillLoad || skillLoad.requested.length === 0) return null;
	const requested = formatSkillList(skillLoad.requested);
	const loaded = formatSkillList(skillLoad.loaded);
	const missing = formatSkillList(skillLoad.missing);
	return `skills req:[${requested}] loaded:[${loaded}] missing:[${missing}]`;
}
...
export function renderResult(
	...
	if (details.mode === "single") {
		...
		if (!first) {
			...
			return new Text(fallback?.type === "text" && fallback.text ? fallback.text : "(no output)", 0, 0);
		}
...
function renderSingleExpanded(
	...
	if (r.skillLoad && r.skillLoad.requested.length > 0) {
		...
		container.addChild(
			new Text(
				theme.fg(
					"dim",
					`lookup cwd: ${shortenPath(r.skillLoad.lookupCwd)}`,
				),
				0,
				0,
			),
		...
		container.addChild(
			new Text(
				theme.fg(
					"dim",
					`requested: ${formatSkillList(r.skillLoad.requested)}`,
				),
				0,
				0,
			),
		...
		container.addChild(
			new Text(
				theme.fg("dim", `loaded: ${formatSkillList(r.skillLoad.loaded)}`),
				0,
				0,
			),
		);
		if (r.skillLoad.missing.length > 0) {
			container.addChild(
				new Text(
					theme.fg(
						"warning",
						`missing: ${formatSkillList(r.skillLoad.missing)}`,
					),
					0,
					0,
				),
	...
	if (displayItems.length === 0 && !finalOutput) {
		const emptyLabel = r.exitCode === -1 ? "(running...)" : "(no output)";
...
function renderSingleCollapsed(
	...
	const skillSummary = getSkillSummary(r);
	if (skillSummary) {
		const skillColor = r.skillLoad && r.skillLoad.missing.length > 0 ? "warning" : "dim";
		text += `\n${theme.fg(skillColor, skillSummary)}`;
	}
...
function renderParallelExpanded(
	...
	for (const r of details.results) {
		...
		if (skillSummary) {
			const skillColor = r.skillLoad && r.skillLoad.missing.length > 0 ? "warning" : "dim";
			container.addChild(new Text(theme.fg(skillColor, skillSummary), 0, 0));
		}
